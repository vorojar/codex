...
if [[ "${RUNNER_OS:-}" == "Windows" ]]; then
  ...
  if [[ $windows_cross_compile -eq 1 ]]; then
    # Use a shell path that also exists on Linux RBE workers. Bazel runs local
    # Windows actions from its execroot, and some Windows path normalization
    # treats `/usr/bin/bash` as rooted on either the workspace or system drive.
    # Put a Git Bash usr/bin shim on both drives so local TestRunner actions
    # and Linux RBE actions can share one `--shell_executable` value.
    bash_path="$(command -v bash)"
    bash_dir="$(dirname "$bash_path")"

    workspace_drive="$(pwd -W | sed 's#^\([A-Za-z]\):.*#\1#')"
    system_drive="${SYSTEMDRIVE:-C:}"
    system_drive="${system_drive%:}"
    for drive in "${workspace_drive,,}" "${system_drive,,}"; do
      portable_shell_dir="/${drive}/usr/bin"
      mkdir -p "$portable_shell_dir"
      cp -R "$bash_dir"/. "$portable_shell_dir/"
      bash_windows_path="$(cygpath -w "$portable_shell_dir/bash.exe")"
      bash_no_ext_windows_path="$(cygpath -w "$portable_shell_dir/bash")"
      powershell.exe -NoProfile -Command "Copy-Item -LiteralPath '${bash_windows_path}' -Destination '${bash_no_ext_windows_path}' -Force"
      "$portable_shell_dir/bash" -lc 'command -v dirname >/dev/null'
    done
    post_config_bazel_args+=("--shell_executable=/usr/bin/bash")
  fi
