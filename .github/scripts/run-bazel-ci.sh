...
if [[ "${RUNNER_OS:-}" == "Windows" ]]; then
  ...
  if [[ $windows_cross_compile -eq 1 ]]; then
    ...
    # Windows actions from the D: execroot, so make the same absolute path work
    # there by copying Git Bash to D:\usr\bin\bash.exe.
    workspace_drive="$(pwd -W | sed 's#^\([A-Za-z]\):.*#\1#')"
    portable_shell_dir="/${workspace_drive,,}/usr/bin"
    mkdir -p "$portable_shell_dir"
    cp "$(command -v bash)" "$portable_shell_dir/bash.exe"
