...
use crate::diagnostics::ignored_toml_value_field;
...
pub async fn load_config_layers_state(
    ...
    let cli_overrides_layer = if cli_overrides.is_empty() {
        ...
        validate_cli_overrides_strictly_if_requested(strict_config, &cli_overrides_layer)?;
        let base_dir = cwd
            .as_ref()
...
fn validate_config_toml_strictly_if_requested(
    ...
    if let Some(config_error) =
        config_error_from_ignored_toml_fields::<ConfigToml>(toml_file, contents)
    {
        return Err(io_error_from_config_error(
            io::ErrorKind::InvalidData,
            config_error,
