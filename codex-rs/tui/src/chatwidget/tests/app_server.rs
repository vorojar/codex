...
async fn blocked_user_prompt_submit_hook_continues_queued_inputs() {
    ...
    handle_hook_completed_for_turn(
        &mut chat,
        Some("turn-2"),
        hook_run(
    ...
    assert_matches!(
        op_rx.try_recv(),
        Ok(Op::AddToHistory { text }) if text == "blocked prompt"
    );
    assert_matches!(op_rx.try_recv(), Err(TryRecvError::Empty));

    handle_user_prompt_submit_stopped(&mut chat, "turn-2");
...
#[tokio::test]
async fn blocked_user_prompt_submit_stopped_only_resumes_queue_once_after_multiple_handlers() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.thread_id = Some(ThreadId::new());
    ...
    for text in ["blocked prompt", "allowed prompt", "later prompt"] {
        chat.bottom_pane
            .set_composer_text(text.to_string(), Vec::new(), Vec::new());
        chat.handle_key_event(KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE));
    }
    ...
    assert_matches!(
        op_rx.try_recv(),
        Ok(Op::AddToHistory { text }) if text == "allowed prompt"
    );
    assert_matches!(op_rx.try_recv(), Err(TryRecvError::Empty));
    assert_eq!(chat.queued_user_message_texts(), vec!["later prompt"]);
}
