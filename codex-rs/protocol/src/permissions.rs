...
impl FileSystemSandboxPolicy {
    ...
    pub fn from_legacy_sandbox_policy(sandbox_policy: &SandboxPolicy) -> Self {
        ...
        Self::workspace_write(writable_roots, *exclude_tmpdir_env_var, *exclude_slash_tmp)
    }
...
mod tests {
    ...
    #[test]
    fn legacy_workspace_write_projection_allows_missing_git_and_agents() {
        let cwd = TempDir::new().expect("tempdir");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: Vec::new(),
            network_access: false,
        ...
        assert!(
            ...
            "legacy workspace-write must allow git init to create .git"
        );
        assert!(
            file_system_policy
                .can_write_path_with_cwd(cwd.path().join(".agents").as_path(), cwd.path()),
            "legacy workspace-write must allow creating .agents"
        );
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(cwd.path().join(".codex").as_path(), cwd.path()),
            "legacy workspace-write still protects .codex before it exists"
        );
    }
