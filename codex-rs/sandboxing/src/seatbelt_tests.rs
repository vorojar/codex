...
#[test]
fn create_seatbelt_args_with_limited_git_writes_carves_out_config_and_hooks() {
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
        dot_git_canonical,
    ...
    let writable_definitions: Vec<&str> = args
        .iter()
        .map(String::as_str)
        .filter(|arg| arg.starts_with("-DWRITABLE_ROOT_"))
        .collect();
    ...
    assert!(
        !writable_definitions
            .iter()
            .any(|arg| arg.ends_with(&format!("={}", dot_git_canonical.to_string_lossy()))),
        "limited Git writes should not carve out the entire .git directory in {writable_definitions:#?}"
    );
