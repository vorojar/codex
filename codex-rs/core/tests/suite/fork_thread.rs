...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    // Fork again with n=0 cuts before the remaining first user message.
    let NewThread {
        thread: codex_fork2,
        ..
    } = thread_manager
...
async fn fork_thread_from_history_does_not_require_source_rollout_path() {
    ...
    let source_items = read_items_materialized(&source_path);
    let NewThread {
    ...
    let forked_items = read_items_materialized(&forked_path);
