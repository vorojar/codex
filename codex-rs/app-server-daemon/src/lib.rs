...
impl Daemon {
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        let (backend, pid) = self.start_managed_backend().await?;
        let info = self.wait_until_ready().await?;
        Ok(self.output(
    ...
    async fn running_backend_instance(&self) -> Result<Option<Backend>> {
        for backend in backend::managed_backends(self.backend_paths.clone()) {
    ...
    async fn start_managed_backend(&self) -> Result<(Backend, Option<u32>)> {
        let backend = backend::preferred_backend(self.backend_paths.clone()).await;
        match backend.start().await {
            Ok(pid) => Ok((backend, pid)),
            Err(systemd_err) if backend.kind() == BackendKind::SystemdUser => {
                let fallback = backend::pid_backend(self.backend_paths.clone());
